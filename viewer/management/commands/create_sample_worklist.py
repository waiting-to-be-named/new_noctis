...
class Command(BaseCommand):
    help = 'Create sample worklist entries for testing'

    def handle(self, *args, **options):
        self.stdout.write("Creating sample data for Noctis worklist...")
        ...
        for i, entry_data in enumerate(sample_entries):
            ...
            self.stdout.write(f"Created worklist entry: {entry.patient_name} - {entry.accession_number}")
            ...
            if entry_data['status'] in ['in_progress', 'completed'] and random.choice([True, False]):
                ...
                self.stdout.write(f"  -> Created associated DICOM study: {study.study_instance_uid}")
        
        self.stdout.write(f"\nSample data creation completed!")
        self.stdout.write(f"Created {len(facilities)} facilities")
        self.stdout.write(f"Created {len(sample_entries)} worklist entries")
        self.stdout.write(f"Created {WorklistEntry.objects.filter(study__isnull=False).count()} associated DICOM studies")
        ...
        for facility in facilities:
            ...
            self.stdout.write(f"  {facility.name}: {count}")
        
        self.stdout.write(self.style.SUCCESS('Successfully created sample worklist data!'))